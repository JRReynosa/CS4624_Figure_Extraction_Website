...
<body>
    ...
    foreach ($results as $r) {
        ...
        $caption = $r['_source']['caption'];
        $figtype = $r['_source']['figType'];
        $imagetext = implode(" ", $r['_source']['imageText']);

        $figure = new Figure();

        $figure->set_id($r['_id']);
        $figure->set_path($r['_source']['renderURL']);

        // Replace strings with highlighted text.
        $figure->set_caption(str_ireplace($temp_array, $replace, $caption));
        $figure->set_figtype(str_ireplace($temp_array, $replace, $figtype));
        $figure->set_imagetext(str_ireplace($temp_array, $replace, $imagetext));
